...
:year: 2024
:org: VSR
:copyright: (C){year} Fynn Conor Donnellan
:classification: {org}
:GH-org: DonFynn
:GH-repo: pi-zero-cluster
:repo-file: README
//
// URLs
...
* link:{url-repo}[Document Github Source ^]
* link:{url-repo-pages-pdf}[Document PDF ^]

[NOTE]
=====
Be aware that github does not properly display this document.
If you want to see the document properly open either the PDF or the web pages above.
=====
...
image::./images/photo-cluster.jpeg[]
...
== raspi-config settings per script
Normally you are asked to set the rasp-config settings for hostname etc interactively with the `rasp-config` tool.

**It's also possible to do it via script.**

.setting raspi-config settings by CLI (non-interactively)
----
...
* link:https://github.com/raspberrypi-ui/rc_gui/blob/master/src/rc_gui.c[^]
...
.Bracket for the Pi Zero
image::./images/3D/RPI_zero_Cluster_mounting_bracket_v2.png[]

.The above bracket was downloaded here:
* link:https://www.thingiverse.com/thing:2502615[]

.Base for the power supply and the USB hub
image::./images/3D/RPI_zero_Cluster_mounting_bracket_power.png[]
...
If you just want to print the designs just open the `.STL` files with a slicer. The slicewr creates gcode files for specific printers.

.Slicer that was used to print the above
* link:https://ultimaker.com/software/ultimaker-cura/[^]

The brackets and the cluster holder where printed on an Ender 3 printer in PLA.
