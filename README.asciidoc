...
The two containers can have different dependencies, such as different versions of python, and they do not interact with each other.
They only communicate over the network.
The cluster spreads the network across the cluster so that containers do not need to run on the same hardware and can be sread across the cluster.

.difference between containers and VMs
image::./images/virtualization.png[]

The reason not to use VMs is that they require too many resources.
Using Containers reduces the amount of required resources so that even a small Pi Zero can be used.
...
We then downloaded docker and set it up witht the help of this video.
...
the following shell script gets copied to the systems below:
...
.short script to run thruogh all 4 systems
[source,BASH]
----
...
.short script to run through all 4 systems
[source,BASH]
----
for system in piZc1 piZc2 piZc3 piZc4
    do
